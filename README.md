...
当前版本已支持**单向同步**（本地推送到云端）：

- **批量同步**：在左侧勾选一个或多个笔记，点击「同步云端」。
- **单条同步**：在笔记编辑页点击「同步云端」或底部「同步当前到云端」。

首次使用前请先在「设置」里填写：

- **云端用户ID**：例如 `windy`、`boss`。  
  同步会写入云端目录 `notefile/<云端用户ID>/`，并自动更新云端 `notes_index.json`，确保云端页面能立即显示。

> 说明：若不填写云端用户ID，同步会被拦截并提示先配置，避免写到错误用户目录。
...
- **提示“云端同步失败 / 云端页面看不到内容”？**  
  ...
  1) 设置里已填写正确的「云端用户ID」；  
  2) 本地后端是最新版本并已重启；  
  3) 云端项目目录与服务实际运行目录一致（当前默认使用 `/var/www/chickennoteLM`）。  
  若仍失败，把 toast 报错全文贴出来即可继续排查。
